...
|  - Blockstore                                            |
|  - Routing（Provide 等）                                 |
|  - Bitswap（僅**提供**本機已有區塊；讀取不經 bitswap 拉塊） |
|  - BlockService（讀取路徑為離線 + Notify→bitswap）       |
|  - DAGService                                            |
...
* 初始化 datastore
* 初始化 blockstore
* 初始化 routing（可選 DHT Provide）
* 初始化 bitswap（`network` + `providerFinder=nil`）+ **serveLocalExchange**（讀取僅命中本地，寫入後 `NotifyNewBlocks` 轉 bitswap 以便**向對等端提供**）
...
**閘道與讀取／提供策略**：讀取路徑（**`GET /ipfs/{cid}`**、`Cat`/`Stat`）僅查本地 blockstore，**不**向其他節點拉取缺失區塊；寫入後透過 **`NotifyNewBlocks`** 讓 **bitswap 伺服器端** 能對已連線對等端 **提供**本機已有區塊。`bitswap.New` 使用 **`providerFinder=nil`**，不向 DHT 做「為拉塊而查 provider」。若啟用 **`auto_provide`**，仍會在 **`AddFile`** 後對 DHT 做 **`Provide`**（宣告本機提供該 CID），與「不拉取」不衝突。
